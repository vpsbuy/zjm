...
install_deps() {
  ...
  if command -v apt-get &>/dev/null; then
    # ── Debian/Ubuntu 自愈 buster-backports 404 ──
    if ! apt-get update -qq; then
      if grep -R "buster-backports" /etc/apt/{sources.list,sources.list.d} 2>/dev/null; then
  ...
  elif command -v dnf &>/dev/null;   then dnf  install -y curl unzip iproute
  elif command -v yum &>/dev/null;   then yum  install -y curl unzip iproute
  elif command -v pacman &>/dev/null;then pacman -Sy --noconfirm curl unzip iproute2
  elif command -v apk &>/dev/null;   then apk add --no-cache curl unzip openrc iproute2
  else
    echo -e "${YELLOW}无法识别包管理器，请手动装 curl / unzip${NC}"; exit 1
  fi
}
...
do_install() {
  ...
  # ── 网卡 ──────────────────────────────
  DEFAULT_IFACE=$(command -v ip &>/dev/null \
                    && ip route | awk '/^default/{print $5;exit}' \
                    || ifconfig 2>/dev/null | awk '/flags=/{print $1;exit}' | sed 's/://')
  [[ -z "$DEFAULT_IFACE" ]] && DEFAULT_IFACE="eth0"
  if [[ $CLI_MODE -eq 1 && -n "$INTERFACE" ]]; then : ; elif [[ $CLI_MODE -eq 1 ]]; then
